...
impl StopWatch {
    pub fn new(regs: TIMER0, top: u32) -> Self {
    ...
    pub fn read_event(&mut self, cc: u32) -> bool {
        if cc < 5 {
            return self.regs.events_compare[cc as usize].read().bits() & 1 != 0;
        }
        false
    }
    ...
    pub fn reset_event(&mut self, cc: u32) {
        if cc < 5 {
            ...
            self.regs.events_compare[cc as usize].write(|w| unsafe { w.bits(0) });
        }
    }
    ...
    pub fn enable_interrupt(&mut self, cc: usize) {
        match cc {
            0 => self.regs.intenset.write(|w| w.compare0().set_bit()),
            1 => self.regs.intenset.write(|w| w.compare1().set_bit()),
            2 => self.regs.intenset.write(|w| w.compare2().set_bit()),
            3 => self.regs.intenset.write(|w| w.compare3().set_bit()),
            // 4 => self.regs.intenset.write(|w| w.compare4().set_bit()),
            // 5 => self.regs.intenset.write(|w| w.compare5().set_bit()),
            _ => (),
        }
    }
    #[inline(always)]
    pub fn set_event(&mut self, cc: usize, val: u32) {
        if cc < 5 {
            // Write zero to clear the event!
            self.regs.tasks_capture[cc].write(|w| unsafe { w.bits(1) });

            // Reprogram CC value
            let cc_val = self.regs.cc[cc].read().bits();
            let cc_val = cc_val.wrapping_add(val) % 1_000_000;
            self.regs.cc[cc as usize].write(|w| unsafe { w.bits(cc_val) });
    ...
    //     if cc < 5 {
    //         // Write zero to clear the event!
    //         self.regs.events_compare[cc].write(|w| unsafe { w.bits(0) });
