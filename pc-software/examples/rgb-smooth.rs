...
use crossterm::event::{poll, KeyEventKind, KeyEventState};
use rand::{self, Rng};
...
fn main() -> crossterm::Result<()> {
    ...
    let ticks = Duration::from_millis(30);

    let mut target_col = RGB8::default();
    let mut current_col = target_col.clone();
    ...
    loop {
        ...
        for d in buf[0..N_LEDS].iter_mut() {
            *d = current_col;
        }
